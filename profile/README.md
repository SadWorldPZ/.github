## 🇬🇧 English Version

### 📌 What is This?
This repository is managed under **GitHub Free**. All contributions must follow the **branch protection rules** and **pull request (PR) workflow**.

## ✅ Contribution Workflow
1. **Clone the repository**:
   ```sh
   git clone https://github.com/organization/repository.git
   cd repository
Create a new feature branch:

sh
Копировать
Редактировать
git checkout -b feature/your-feature-name
Make your changes and commit them:

sh
Копировать
Редактировать
git add .
git commit -m "Description of your changes"
Push to GitHub:

sh
Копировать
Редактировать
git push origin feature/your-feature-name
Create a Pull Request (PR)

Go to the GitHub repository.
Open a new Pull Request.
Wait for manual review from at least one team member before merging.
🔐 Branch Protection Rules (GitHub Free)
✅ All changes must go through Pull Requests (PRs).
✅ Force pushes to main are blocked.
❌ PR approvals are NOT enforced (Follow the process manually).
❌ Automated status checks are NOT required by default (Can be configured manually).
🔧 GitHub Actions & CI/CD (Limited)
On GitHub Free, GitHub Actions are available but limited to 2000 minutes per month.

🚀 Mod validation (if applicable, but limited by usage quotas)
✅ Code style checks (if set up manually in CI/CD)
🔄 Automatic deployment (optional, but usage is limited)
⚠️ GitHub Free Limitations
No enforced PR approvals – Manual approval is required.
No Advanced Security features (code scanning, secret scanning).
Private repositories only allow up to 3 collaborators.
Limited GitHub Actions minutes per month (2000 minutes).
📞 Need Help?
For any issues, please contact the repository maintainers or open an issue.

🇷🇺 Русская версия
📌 Описание
Этот репозиторий управляется в GitHub Free. Все изменения должны следовать правилам защиты веток и процессу Pull Request (PR).

✅ Алгоритм работы
Клонируйте репозиторий:

sh
Копировать
Редактировать
git clone https://github.com/organization/repository.git
cd repository
Создайте новую ветку для изменений:

sh
Копировать
Редактировать
git checkout -b feature/название-фичи
Внесите изменения и зафиксируйте их:

sh
Копировать
Редактировать
git add .
git commit -m "Описание изменений"
Отправьте изменения в GitHub:

sh
Копировать
Редактировать
git push origin feature/название-фичи
Создайте Pull Request (PR)

Перейдите в репозиторий на GitHub.
Откройте новый Pull Request.
Дождитесь ручного одобрения от команды перед слиянием.
🔐 Правила защиты веток (GitHub Free)
✅ Все изменения должны проходить через Pull Request (PR).
✅ Запрещены force push в main.
❌ Ограничение PR одобрений не применяется автоматически (нужно следовать процессу вручную).
❌ Статус-проверки не обязательны по умолчанию (можно настроить вручную).
🔧 GitHub Actions & CI/CD (Ограничения)
На GitHub Free можно использовать GitHub Actions, но есть лимит 2000 минут в месяц.

🚀 Валидация мода (если настроено, но ограничено по использованию)
✅ Проверка стиля кода (если вручную настроены CI/CD)
🔄 Автоматический деплой (опционально, но использование ограничено)
⚠️ Ограничения GitHub Free
Отсутствует автоматическое требование одобрения PR – требуется ручная проверка.
Нет расширенной безопасности (code scanning, secret scanning).
Приватные репозитории ограничены 3-мя участниками.
Ограничены минуты работы GitHub Actions (2000 минут в месяц).
📞 Нужна помощь?
По вопросам обращайтесь к поддержке репозитория или создайте issue.
